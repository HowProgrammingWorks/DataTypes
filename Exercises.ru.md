...
2. C сигнатурой `inc(n: number): number`, переданный в фунцию аргумент увеличивается на 1,
пример вызова: `const a = 5; const b = inc(a); console.dir({ a, b });`
3. C сигнатурой `inc(num: Num)`, где `Num` является объектом с полем `n`,
чтобы функция изменила поле исходного объекта переданного по ссылке, n увеличивается на 1,
